...
import { flatHierarchyMap, singleChildHierarchyMap } from 'testAssets/HierarchyList';

Enzyme.configure({ adapter: new Adapter() });

it('renders flat hierarchy list without error', () => {
  const div = document.createElement('div');
  ReactDOM.render(<HierarchyList 
    rootHierarchy={0}
    hierarchies={flatHierarchyMap} />, div);
});
...
it('produces correct amount of rows', () => {
  const hList = mount(<HierarchyList 
    rootHierarchy={0}
    onRowExpand={() => {}}
...
it('produces a expand arrow when children are supplied', () => {
  const hListRoot = mount(<HierarchyList 
    rootHierarchy={0}
...
it('fires rowExpand callback when expand button is clicked', () => {
  ...
  const hListRoot = mount(<HierarchyList 
    rootHierarchy={0}
    onRowExpand={() => {rowExpandClicked = true }}
    hierarchies={singleChildHierarchyMap} />);
  const hListChild = mount(<HierarchyList 
    onRowExpand={() => {}}
    hierarchies={singleChildHierarchyMap}
    rootHierarchy={1}
  />);
