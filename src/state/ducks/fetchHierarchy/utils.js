...
function getPayload(data: [string, mixed][]): I.List<HierarchyPayload> {
    return I.List(data.map((payload) => { return { type: 'string', payload: payload[1] } }));
}
...
function makeHierarchy(record: any, hierarchyIndex: number, map: Map<number, HierarchyRecord>): number {
    const payload = []
    record.forEach((element, rowIndex) => {
        ...
        if (childRecord) {
            payload.push(row.set('childId', 
                makeHierarchy(childRecord, getCantorPairing(hierarchyIndex, rowIndex + 1), map)))
        } else {
            payload.push(row)
        }
    })
    map.set(hierarchyIndex, newHierarchy({ 
        headers: getHeaders(record), hierarchyIndex, payload: I.List(payload) }))
