...
export type HierarchyAction =
  ...
  | {
      type: "app/hierarchy/DELETE_ROW",
      hierarchyIndex: number,
      rowIndex: number,
    };

export default function reducer(
  state: Map<number, HierarchyRecord> = Map(),
  action: Action
) {
  switch (action.type) {
    case EXPAND_ROW: {
      ...
      if (hierarchy != null) {
        const rowIndex = hierarchy.payload.findKey(
          (row) => action.rowIndex === row.rowIndex
        );
        const newList = hierarchy.payload.setIn(
          [rowIndex, "expanded"],
    ...
    case DELETE_ROW: {
      return updateExpansionAndChildIds(
        deleteHierarchyIfRowsEmpty(
          withoutRow(state, action.hierarchyIndex, action.rowIndex),
          action.hierarchyIndex
        )
      );
    }
    default:
