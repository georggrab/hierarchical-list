...
export const newPayloadRow: RecordFactory<HierarchyRow> = Record({
    columns: List(), expanded: false, rowIndex: -1, childId: undefined,
});
...
export const newHierarchy: RecordFactory<Hierarchy> = Record({
    headers: List(), payload: List(), hierarchyIndex: -1,
})

export type RowExpandCallback = (hierarchyIndex: number, rowIndex: number, state: boolean) => void;